...
# Extract HCC numbers (this picks up a few extra numbers, 1,2 which are dupes anyway)
hccnum <- gsub("([^0-9])", "", labels[,1])
...
labels <- labels[!is.na(labels$hccnum) & labels$hccnum < 300, ]
...
pt <- pt %>%
  left_join(icdcc) %>%
  select(-icd, -icdversion, -year) %>%
  filter(!is.na(cc)) %>%
  distinct(id, admtdate, cc) %>%
  mutate(cc = as.numeric(cc)) %>%
  group_by(id, admtdate) %>%
  arrange(cc)
```
