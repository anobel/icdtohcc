...
# reshape into long using base functions
# can be done faster using tidyr, but want to minimize dependencies
# pt <- pt %>%
#   gather(diag, icd, -id, -admtdate, na.rm=T)

# will implement icd_wide_to_long from icd package
pt <- reshape(pt,
              varying=diags,
              v.names="icd",
              timevar="diag_num",
...
# drop NA icds generated by reshaping
pt <- pt[!is.na(pt$icd),]
